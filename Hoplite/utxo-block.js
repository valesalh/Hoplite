...
module.exports = class UtxoBlock extends Block {
  ...
  addTransaction(tx, client) {
    ...
    // Adding valid transaction to block
    this.transactions.set(tx.id, tx);

    //Deleting all accounts used as inputs
    tx.from.forEach(address => {
      if(client) client.log(`***Deleting ${address}.`);
      this.balances.delete(address);
    });

    // Updating balances
    tx.outputs.forEach(({amount, address}) => {
      if(client) client.log(`***Giving ${amount} to ${address}.`);
